        ...
        try:
            ...
            if "CRC" in error_msg or "Bad CRC" in error_msg or "corrupted" in error_msg.lower():
                logging.error(
                    ...
                    f"  python scripts/download_data.py {dataset.__name__.lower()} --n_procs {args.n_procs}"
                )
            # Provide helpful message for SSL/ContentLength errors
            elif "SSL" in error_msg or "ContentLength" in error_msg or "SSLError" in error_msg or "ClientPayloadError" in error_msg:
                logging.error(
                    f"SSL/ContentLength error detected for {dataset.__name__}. "
                    f"This usually means network issues or corrupted download.\n"
                    f"Suggestions:\n"
                    f"  1. Try with lower concurrency (recommended for large files):\n"
                    f"     python scripts/download_data.py {dataset.__name__.lower()} --n_procs 1\n"
                    f"  2. Clean cache and retry:\n"
                    f"     rm -rf ~/.cache/huggingface/datasets/*{dataset.__name__.lower()}*\n"
                    f"     python scripts/download_data.py {dataset.__name__.lower()} --n_procs {min(args.n_procs, 2)}"
                )
