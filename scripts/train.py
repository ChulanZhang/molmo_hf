...
def main(cfg: TrainConfig) -> None:
    ...
    # Convert ModelConfig to MolmoConfig for MolmoModel
    molmo_config = model_config_to_molmo_config(cfg.model)
    ...
    if cfg.model.vision_backbone is not None and not cfg.ft_connector:
        freeze_parameters_by_name(olmo_model, MolmoModel.get_connector_parameters(), warn=False)
    if cfg.model.vision_backbone is not None and not cfg.ft_vit:
        log.info(f"Freezing vision backbone")
        freeze_parameters_by_name(olmo_model, MolmoModel.get_vit_parameters(), warn=False)
    if not cfg.ft_llm:
        log.info(f"Freezing LLM")
        freeze_parameters_by_name(olmo_model, MolmoModel.get_llm_parameters(), warn=False)
    ...
    if cfg.load_path is None:
        ...
        if get_local_rank() == 0:
            ...
            if cfg.initial_model_checkpoint:
                checkpoint_path = Path(cfg.initial_model_checkpoint) / "model.pt"
