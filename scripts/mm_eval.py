...
from molmo.checkpoint import load_model_state
from molmo.config import EvalConfig, TokenizerConfig, ModelConfig, DatasetEvaluatorConfig, \
    VisionBackboneConfig
from molmo.data import build_torch_mm_eval_dataloader
from molmo.eval.inf_evaluator import InfDatasetEvaluator, build_inf_evaluator
...
class ModelEvaluator:
    ...
    def initialize_and_load_model(self) -> MolmoModel:
        cfg = self.config
        ...
        if cfg.load_path == "debug":
            ...
            molmo_config = model_config_to_molmo_config(model_cfg)
            olmo_model = MolmoModel(molmo_config).to(device)
            # Note: reset_parameters is not implemented in MolmoModel, using reset_with_pretrained_weights instead
            ...
            hf_model = MolmoForCausalLM.from_pretrained(
                cfg.load_path[3:], trust_remote_code=True, torch_dtype='fp32', device_map='cpu')
            olmo_model = hf_model.model  # Get the underlying MolmoModel
            model_cfg = olmo_model.config
        elif cfg.fsdp is None:
            log.info("Loading model without FSDP...")
            # Load from checkpoint - need to convert ModelConfig to MolmoConfig
            ...
            molmo_config = model_config_to_molmo_config(model_cfg)
            olmo_model = MolmoModel(molmo_config)
            # Load state dict
        ...
        # Get dtype from embedding weights
        if hasattr(olmo_model.transformer.wte, 'embedding'):
            dtype = olmo_model.transformer.wte.embedding.dtype
        else:
            dtype = olmo_model.transformer.wte.weight.dtype
