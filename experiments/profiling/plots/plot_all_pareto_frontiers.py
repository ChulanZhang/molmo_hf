"""
Batch script to generate Pareto frontier plots for all exp5 and exp6 results.
Generates plots for all datasets and all latency types (prefill, decode, total).
"""
...
def load_exp5_results(exp5_dir: str) -> Dict[Tuple[int, int, int], float]:
    ...
    # Find all exp5 result files (exclude the merged results file)
    pattern = os.path.join(exp5_dir, "exp5_accuracy_results_*.json")
    files = glob.glob(pattern)
    
    # Filter out the merged results file, but include rank0 files if no non-rank0 files exist
    non_rank0_files = [f for f in files if "rank0" not in f and not f.endswith("exp5_accuracy_results.json")]
    rank0_files = [f for f in files if "rank0" in f and not f.endswith("exp5_accuracy_results.json")]
    
    # Prefer non-rank0 files, but use rank0 files if that's all we have
    if len(non_rank0_files) > 0:
        files = non_rank0_files
    else:
        files = rank0_files
...
def plot_accuracy_latency_pareto(
    ...
    """
    print(f"\n{'='*80}")
    print(f"Plotting {dataset_name} - {latency_type} latency ({latency_metric})")
    print(f"{'='*80}")
    ...
    ax.set_title(f'{dataset_name}: Accuracy vs {latency_label} Trade-off (Pareto Frontier)', fontsize=18, fontweight='bold')
    ax.grid(True, alpha=0.3)
    ax.legend(loc='upper left', fontsize=14)
    ax.tick_params(labelsize=14)
    
    # Add text box explaining the three knobs (bottom right corner, left-aligned text)
    knob_explanation = (
        "Control Knobs:\n"
    ...
    # Use right alignment so the text box stays within bounds
    ax.text(
        0.98, 0.02, knob_explanation,
    ...
    # Improve layout - adjust padding to ensure text box is visible
...
def generate_all_pareto_plots(
    results_base_dir: str = "/home/x-pwang1/ai_project/molmo_hf/results/profiling",
    latency_metric: str = "mean",
    dpi: int = 300,
):
    """
    Generate Pareto frontier plots for all datasets and latency types.
    
    Args:
        results_base_dir: Base directory containing exp5 and exp6 results
        latency_metric: Which latency metric to use ("mean", "P50", "P95", "P99")
    ...
    for dataset in datasets:
        exp5_dir = os.path.join(results_base_dir, f"exp5_accuracy_{dataset}")
        exp6_file = os.path.join(results_base_dir, f"exp6_latency_{dataset}", "exp6_latency_results.json")
        
        # Check if directories exist
        if not os.path.exists(exp5_dir):
            print(f"Warning: exp5 directory not found: {exp5_dir}")
            continue
        if not os.path.exists(exp6_file):
            print(f"Warning: exp6 file not found: {exp6_file}")
            continue
        
        # Create figures directory
        figures_dir = os.path.join(results_base_dir, f"exp6_latency_{dataset}", "figures")
        os.makedirs(figures_dir, exist_ok=True)
        
        # Generate plots for each latency type
        for latency_type in latency_types:
            output_file = os.path.join(figures_dir, f"pareto_frontier_{latency_type}_latency.png")
...
def generate_orin_pareto_plots(
    results_base_dir: str = "/home/x-pwang1/ai_project/molmo_hf/results/profiling",
    orin_dir: str = "/home/x-pwang1/ai_project/molmo_hf/results/profiling/orin",
    latency_metric: str = "mean",
    dpi: int = 300,
):
    """
    Generate Pareto frontier plots for Orin (using vqa2 accuracy and orin latency).
    
    Args:
        results_base_dir: Base directory containing exp5 accuracy results
        orin_dir: Directory containing orin latency results
        latency_metric: Which latency metric to use ("mean", "P50", "P95", "P99")
        dpi: DPI for the figures
    """
    # Use vqa2 accuracy results
    exp5_dir = os.path.join(results_base_dir, "exp5_accuracy_vqa2")
    exp6_file = os.path.join(orin_dir, "exp6_latency", "exp6_latency_results.json")
    ...
    if not os.path.exists(exp5_dir):
        ...
        return
    if not os.path.exists(exp6_file):
        print(f"Warning: orin exp6 file not found: {exp6_file}")
        return
    
    # Create figures directory
    figures_dir = os.path.join(orin_dir, "exp6_latency", "figures")
    os.makedirs(figures_dir, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Pareto frontier plots for all exp5 and exp6 results"
    )
    parser.add_argument(
        "--results_base_dir",
        type=str,
        default="/home/x-pwang1/ai_project/molmo_hf/results/profiling",
        help="Base directory containing exp5 and exp6 results"
    )
    parser.add_argument(
        "--orin_dir",
        type=str,
        default="/home/x-pwang1/ai_project/molmo_hf/results/profiling/orin",
        help="Directory containing orin latency results"
    )
    parser.add_argument(
        "--latency_metric",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="DPI for the output figures (default: 300)"
    )
    parser.add_argument(
        "--orin_only",
        action="store_true",
        help="Only generate plots for Orin"
    )
    parser.add_argument(
        "--server_only",
        action="store_true",
        help="Only generate plots for server datasets"
    )
    
    args = parser.parse_args()
    
    if not args.orin_only:
        print("\n" + "="*80)
        print("Generating Pareto frontier plots for all server datasets...")
        print("="*80)
    ...
    if not args.server_only:
        print("\n" + "="*80)
        print("Generating Pareto frontier plots for Jetson Orin...")
        print("="*80)
        generate_orin_pareto_plots(
            results_base_dir=args.results_base_dir,
            orin_dir=args.orin_dir,
            latency_metric=args.latency_metric,
            dpi=args.dpi,
        )
