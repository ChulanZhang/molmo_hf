...
class MolmoeSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        # selected_experts shape: (batch * seq_len, top_k), values are expert indices (0 to num_experts-1)
        # one_hot shape: (batch * seq_len, top_k, num_experts)
        # permute(2, 1, 0) shape: (num_experts, top_k, batch * seq_len)
        ...
        for expert_idx in range(self.num_experts):
            ...
            routing_weights_selected = routing_weights_rows.gather(1, idx.unsqueeze(1)).squeeze(1)

            current_hidden_states = expert_layer(current_state) * routing_weights_selected.unsqueeze(-1)
