...
# ============================================================================
# Configuration Bridge Functions: MolmoConfig (HF) <-> ModelConfig (Training)
# ============================================================================

def molmo_config_to_model_config(molmo_config):
    """
    Convert a HuggingFace MolmoConfig to a training ModelConfig.
    
    Args:
        molmo_config: An instance of molmo.models.config_molmoe.MolmoConfig
        
    Returns:
        An instance of ModelConfig suitable for training
    """
    from molmo.models.config_molmoe import MolmoConfig as HFMolmoConfig
    
    if not isinstance(molmo_config, HFMolmoConfig):
        raise TypeError(f"Expected MolmoConfig, got {type(molmo_config)}")
    
    # Get all attributes from the HF config
    config_dict = molmo_config.to_dict()
    
    # Handle vision_backbone separately if it exists
    vision_backbone = None
    if "vision_backbone" in config_dict and config_dict["vision_backbone"] is not None:
        vb_dict = config_dict.pop("vision_backbone")
        vision_backbone = VisionBackboneConfig(**vb_dict)
    
    # Create ModelConfig with the converted values
    # Filter out HF-specific keys that don't exist in ModelConfig
    model_config_dict = {}
    for key, value in config_dict.items():
        # Skip HF-specific keys
        if key in ["transformers_version", "model_type", "architectures", "use_cache", 
                   "auto_map", "pad_token_id"]:
            continue
        # Map some keys if needed
    ...
    # Set vision_backbone if it exists
    if vision_backbone is not None:
        model_config.vision_backbone = vision_backbone
    
    return model_config


def model_config_to_molmo_config(model_config, use_cache=False):
    """
    Convert a training ModelConfig to a HuggingFace MolmoConfig.
    ...
    Returns:
        An instance of MolmoConfig suitable for HF integration
    """
    from molmo.models.config_molmoe import MolmoConfig as HFMolmoConfig
    
    if not isinstance(model_config, ModelConfig):
        raise TypeError(f"Expected ModelConfig, got {type(model_config)}")
    
    # Convert to dict
    config_dict = asdict(model_config)
    
    # Handle vision_backbone separately
    if "vision_backbone" in config_dict and config_dict["vision_backbone"] is not None:
        vb = config_dict.pop("vision_backbone")
        if isinstance(vb, VisionBackboneConfig):
            config_dict["vision_backbone"] = asdict(vb)
    
    # Add HF-specific keys
    config_dict["use_cache"] = use_cache
    config_dict["model_type"] = "molmo"
    config_dict["architectures"] = ["OLMoForCausalLM"]
    
    # Create MolmoConfig instance
    molmo_config = HFMolmoConfig(**config_dict)
    
    return molmo_config


def load_model_config_from_hf_config(config_path_or_dict):
    """
    Load a ModelConfig from a HuggingFace config.json file or dict.
    
    Args:
        config_path_or_dict: Path to config.json file or a dict with config values
    ...
    if isinstance(config_path_or_dict, str):
        # Load from file
        hf_config = HFMolmoConfig.from_pretrained(config_path_or_dict)
    elif isinstance(config_path_or_dict, dict):
        # Load from dict
        hf_config = HFMolmoConfig(**config_path_or_dict)
    else:
        # Assume it's already a MolmoConfig
        hf_config = config_path_or_dict
    
    return molmo_config_to_model_config(hf_config)
