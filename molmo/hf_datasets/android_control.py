...
from molmo.hf_datasets.android_control_utils import *
...
def process_data(data_files: Dict[str, str]):
    ...
    try:
        ...
        raise ImportError(
            "Unable to import android control protos. AndroidControl dataset requires android_env to be installed.\n"
            "To install:\n"
            "  1. Clone the repository: git clone https://github.com/google-deepmind/android_env.git\n"
            "  2. Install it: cd android_env && pip install -e .\n"
            "  3. Add to PYTHONPATH: export PYTHONPATH=$PYTHONPATH:$(pwd)\n"
            "  4. Also install required dependencies: pip install tensorflow tensorflow_datasets pillow protobuf\n"
            "Alternatively, you can skip this dataset using --ignore_errors"
        )
